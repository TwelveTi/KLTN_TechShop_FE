...
import type { AuthForm, AuthMode, AuthPayload, ThemeMode } from '../types'
import { useNavigate, useParams, useSearchParams } from '@core/router'
import { paths, type AuthMode as AuthRouteMode } from '@routes/paths'
 ...
 *
 * Chế độ nằm trong URL (`/auth/register`), nên link chia sẻ mở đúng biểu mẫu và
 * nút Back đi ngược qua các bước của luồng khôi phục. v1 giữ chế độ trong
 * `useState` với một prop `initialMode`.
 */
export function AuthScreen() {
  const navigate = useNavigate()
  const routeParams = useParams<{ mode?: AuthRouteMode }>()
  const [params] = useSearchParams()
  const initialMode = (routeParams.mode ?? 'login') as AuthMode
  const onAuthenticated = () => navigate(paths.home())
  const onLocalLogout = () => navigate(paths.home())
  const onBrowseHome = () => navigate(paths.home())
  const { showToast } = useToast()
  // Chế độ SỐNG TRONG URL, không phải useState: /auth/register và /auth/login
  // là hai địa chỉ khác nhau, nên link chia sẻ mở đúng biểu mẫu và nút Back đi
  // ngược qua các bước của luồng khôi phục mật khẩu.
  const mode = initialMode
  const setMode = (next: AuthMode) => navigate(paths.auth(next as AuthRouteMode))
  ...
  // Phiên do AuthProvider sở hữu — AuthScreen chỉ đọc và gọi hành động.
  const { authResult, signIn, clearLocalSession } = useAuth()

  /**
   * Thông báo đến từ query param của lần quay lại (OAuth, xác minh email).
   * Suy ra lúc render: URL là nguồn sự thật, nên không cần một effect gọi
   * `setState` — và Back/Forward tự cho ra đúng thông báo.
   */
  const urlNotice = useMemo(() => {
    if (params.get('oauth') === 'error') {
      return {
        error: 'Google sign-in did not complete. Please try again or sign in with your email.',
        success: '',
      }
    }
    switch (params.get('verified')) {
      case 'success':
        return { error: '', success: 'Email verified successfully. You may now sign in.' }
      case 'already':
        return { error: '', success: 'Email is already verified. Please sign in to continue.' }
      case 'error':
        return { error: 'Email verification link is invalid or has expired.', success: '' }
      default:
        return { error: '', success: '' }
    }
  }, [params])

  // Hiển thị = lỗi/thông báo của form, nếu không có thì lấy từ query param.
  const error = formError || urlNotice.error
  const success = formSuccess || urlNotice.success
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (mode === 'forgot-password') {
        ...
        setFormSuccess('Code verified. Choose your new password.')
        ...
        setForm(initialForm)
        setMode('login')
        setFormSuccess('Password updated. You may now sign in with your new credentials.')
      } else if (mode === 'login') {
        ...
        signIn(result)
        setFormSuccess('Authentication verified. Welcome back.')
        onAuthenticated()
        ...
        signIn(result)
        setFormSuccess('Account created successfully. Welcome to TechShop.')
        onAuthenticated()
      }
    } catch (err: unknown) {
      const message = toErrorMessage(err, 'Authentication request failed. Please check your details and try again.')
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      const message = toErrorMessage(err, 'Unable to resend the code right now.')
  ...
  const handleGoogleLogin = () => {
    ...
    setFormSuccess('')
    if (!env.apiBaseUrl) {
      setFormError('API Base URL is not configured. Please check frontend environment settings.')
  ...
  const handleLogout = () => {
    clearLocalSession()
    onLocalLogout()
    setFormSuccess('Local session cleared.')
  }

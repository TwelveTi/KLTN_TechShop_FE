...
import { getMyAddresses, createMyAddress, type CreateAddressPayload, type UserAddress } from '../../profile/api/profileApi'
...
export function CheckoutPage({
  ...
  const { items: cartItems, isSyncing, clearCart } = useCart()
  ...
  const items = useMemo(() => {
    ...
    if (!selection || selection.length === 0) return cartItems
    return cartItems.filter((item) => selection.includes(item.id))
  }, [cartItems])

  const subtotal = useMemo(() => sumSubtotal(items), [items])
  const shippingFee = useMemo(() => resolveShippingFee(subtotal, STANDARD_DELIVERY), [subtotal])
  const total = subtotal + shippingFee

  // Load saved addresses once signed in; preselect the default.
  useEffect(() => {
    ...
    getMyAddresses()
      .then((list) => {
        ...
        setSelectedAddressId((current) => current ?? preferred?.id ?? null)
      })
      .catch(() => {
  ...
  const handleCreateAddress = async (payload: CreateAddressPayload) => {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : 'Could not save the address.', { variant: 'error' })
      throw error
    } finally {
  ...
  const handlePlaceOrder = async () => {
    ...
    setIsPlacing(true)
    try {
      const signature = `${selectedAddressId}|${items.map((i) => `${i.id}:${i.quantity}`).join(',')}`
...
// Content wrapper only — the shared shop header/footer come from UserLayout so
